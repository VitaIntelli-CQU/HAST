...
def clustering(adata, n_clusters=7, radius=50, key='emb', method='mclust', start=0.1, end=3.0, increment=0.01, refinement=False, random_state=42):
    """\
    ...
    hidden_embedding = pca.fit_transform(adata.obsm['hidden_feat'].copy())
    adata.obsm['hidden_emb'] = adata.obsm['hidden_feat'].copy()
    adata.obsm['hidden_emb_pca'] = hidden_embedding
    ...
    if refinement:  
       new_type = weighted_refine_label(adata, radius, key='domain')
...
def weighted_refine_label(adata, radius=50, key='label'):
    n_neigh = radius
    new_type = []
    old_type = adata.obs[key].values
    position = adata.obsm['spatial']
    ...
    for i in range(n_cell):
        vec = distance[i, :]
        index = vec.argsort()[1:n_neigh+1]
        neigh_type = old_type[index]

        weights = 1 / (vec[index] + 1e-6)
        weighted_count = Counter()
        
        for label, weight in zip(neigh_type, weights):
            weighted_count[label] += weight

        max_type = max(weighted_count, key=weighted_count.get)
        new_type.append(max_type)
